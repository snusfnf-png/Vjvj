...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    ...
    Насыщенность (S) и яркость (V) оригинала сохраняются полностью —
    тени остаются тёмными, блики светлыми, чёрные контуры не трогаются.
    """
    import colorsys
    h = hex_color.lstrip("#")
    ...
    # S и V оригинала — сохраняем
    v = cmax
    s = np.where(cmax > 0.001, delta / cmax, 0.0)

    # Пиксели с низкой насыщенностью (чёрные, белые, серые) почти не трогаем
    color_weight = np.clip((s - 0.15) / 0.25, 0.0, 1.0)

    # Собираем новый RGB из (target_hue, s_orig, v_orig)
    ...
    # Смешиваем: нейтральные пиксели не меняются
