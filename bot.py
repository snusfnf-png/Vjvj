...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    """
    Saturation-aware luminance-preserving recolor.

    Логика:
    - Вычисляем насыщенность каждого пикселя (HSV).
    - Цветные пиксели (высокая насыщенность) перекрашиваются в target-цвет
      с сохранением яркости.
    - Нейтральные пиксели (чёрные тени, белые блики, серые контуры) не трогаются —
      weight плавно нарастает от 0 до 1 по насыщенности.
    - Это сохраняет детали и даёт чистый результат без чёрных пятен.
    """
    ...
    # Насыщенность по HSV: sat = (max - min) / max
    cmax = np.maximum(np.maximum(r, g), b)
    cmin = np.minimum(np.minimum(r, g), b)
    sat = np.where(cmax > 0, (cmax - cmin) / cmax, 0.0)

    # weight: насколько сильно красим этот пиксель
    # Нейтральные (серые/чёрные/белые) пиксели — weight близок к 0
    sat_low, sat_high = 0.12, 0.35
    weight = np.clip((sat - sat_low) / (sat_high - sat_low), 0.0, 1.0)

    # Перекрашенный вариант
    tinted_r = np.clip(tr * lum, 0, 255)
    tinted_g = np.clip(tg * lum, 0, 255)
    tinted_b = np.clip(tb * lum, 0, 255)

    # Смешиваем оригинал и перекрашенный по weight
    out_arr = arr.copy()
    out_arr[:, :, 0] = r * (1 - weight) + tinted_r * weight
    out_arr[:, :, 1] = g * (1 - weight) + tinted_g * weight
    out_arr[:, :, 2] = b * (1 - weight) + tinted_b * weight
...
def _make_new_pack_name(bot_username: str, uid: int, pack_name: str, hex_color: str) -> str:
    """
    Формирует уникальное имя нового пака.
    Telegram требует: только a-z, 0-9, _, длина 1-64, должен заканчиваться на _by_<botname>.
    """
    color_suffix = hex_color.lstrip("#").lower()
    # берём первые 20 символов исходного имени, приводим к нижнему регистру
...
async def do_pack(bot: Bot, ctx, state: FSMContext, hex_color: str) -> None:
    ...
    try:
        ...
        new_pack_name = _make_new_pack_name(bot_username, uid, pack_name, hex_color)
        new_pack_title = f"{pack.title} [{hex_color}]"[:64]

        # Определяем тип пака (статичный или анимированный)
        sticker_format = "animated" if is_animated else "static"
        ...
        for i, s in enumerate(stickers):
            ...
            try:
                ...
                sticker_file = BufferedInputFile(data_out, filename=fname)
                emoji = s.emoji or "⭐"

                input_sticker = InputSticker(sticker=sticker_file, emoji_list=[emoji], format=sticker_format)

                if not pack_created:
                    # Создаём новый пак с первым стикером
            ...
            await asyncio.sleep(0.3)   # rate-limit: Telegram не любит слишком быстрое добавление стикеров
        ...
        if pack_created:
            ...
            await bot.send_message(
                chat_id,
                f'<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> '
                f'Не удалось перекрасить ни одного стикера из пака.',
                parse_mode=ParseMode.HTML,
            )
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        f'• Целые стикер-паки и эмодзи-паки → ссылка на новый пак\n\n'
